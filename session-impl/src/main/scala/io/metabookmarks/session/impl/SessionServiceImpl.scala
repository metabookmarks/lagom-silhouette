...
class SessionServiceImpl(persistentEntityRegistry: PersistentEntityRegistry) extends SessionService {
  override def getLoginInfo(id: String) = authenticated(userId => ServerServiceCall {
    loginInfo =>
      val ref = persistentEntityRegistry.refFor[SocialProfileEntity](id)
      ref.ask(GetLoginInfo(id))
  })

  override def getOrCreateLoginInfo = authenticated(loginInfo => ServerServiceCall {
    loginInfo =>
  ...
  override def sessionsTopic(): Topic[api.SessionEvent] =
    TopicProducer.singleStreamWithOffset {
      fromOffset =>
        persistentEntityRegistry.eventStream(SessionEvent.Tag, fromOffset)
  ...
  override def getAuthInfo(id: String) = authenticated(userId => ServerServiceCall {
    loginInfo =>
  ...
  override def addAuthInfo(id: String) = authenticated(userId => ServerServiceCall {
    payload =>
      val ref = persistentEntityRegistry.refFor[AuthInfoEntity](id)
  ...
  override def updateAuthInfo(id: String) = authenticated(userId => ServerServiceCall {
    payload =>
      val ref = persistentEntityRegistry.refFor[AuthInfoEntity](id)
  ...
  override def saveAuthInfo(id: String) = authenticated(userId => ServerServiceCall {
    payload =>
      val ref = persistentEntityRegistry.refFor[AuthInfoEntity](id)
  ...
  override def deleteAuthInfo(id: String) = authenticated(userId => ServerServiceCall {
    _ =>
      val ref = persistentEntityRegistry.refFor[AuthInfoEntity](id)
