  ...
  def submit = silhouette.UnsecuredAction.async {
    ...
    ForgotPasswordForm.form.bindFromRequest.fold(
      ...
      email => {
        ...
        val result = Redirect(io.metabookmarks.lagom.silhouette.controllers.routes.SilhouetteSignInController.view()).flashing("info" -> Messages("reset.email.sent"))
        userService.retrieve(loginInfo).flatMap {
          case Some(user)  =>
            authTokenService.create(email).map { authToken =>
              val url = io.metabookmarks.lagom.silhouette.controllers.routes.ResetPasswordController.view(authToken.id).absoluteURL()
