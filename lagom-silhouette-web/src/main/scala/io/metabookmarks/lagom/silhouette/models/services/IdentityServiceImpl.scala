...
class IdentityServiceImpl @Inject() (sessionService: SessionService, userService: UserService)(
  ...
  def save(silhouetteUser: SilhouetteUser, loginInfo: LoginInfo): Future[SilhouetteUser] =
    if (silhouetteUser.email.isEmpty) {
      ...
      val userToCreate = User(
        ...
        firstName = silhouetteUser.firstName,
        lastName = silhouetteUser.lastName,
  ...
  def save(profile: CommonSocialProfile): Future[SilhouetteUser] =
    ...
    }.getOrElse(Future.failed(new Exception("No email")))

  private def fromLoginInfo[S](loginInfo: LoginInfo)(f: Option[sessionApi.SocialProfileInfo] => Future[S]) =
    sessionService
      .getLoginInfo(s"${loginInfo.providerID}(${loginInfo.providerKey})")
      .secureInvoke()
      .flatMap(f)
